...
type QRProps = { size?: number; seed?: string }

const qrCache: Record<string, boolean[][]> = {}
function qrMatrix(seed: string = 'vior', n: number = 25): boolean[][] {
  const cached = qrCache[seed + n]
  if (cached) return cached
  let h = 2166136261
  for (let i = 0; i < seed.length; i++) { h ^= seed.charCodeAt(i); h = Math.imul(h, 16777619) }
  const rnd = (): number => { h ^= h << 13; h ^= h >>> 17; h ^= h << 5; return ((h >>> 0) % 1000) / 1000 }
  ...
  const finder = (r: number, c: number): void => {
    for (let i = -1; i <= 7; i++) for (let j = -1; j <= 7; j++) {
      ...
      const row = m[rr]!
      row[cc] = (i >= 0 && i <= 6 && j >= 0 && j <= 6) ? (edge || core) : false
    }
  }
...
export default function QR({ size = 196, seed = 'vior' }: QRProps): React.JSX.Element {
