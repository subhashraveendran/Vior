(function () {
    ...
    // Bootstrap pair code from URL (?pair=ABCDEF) or localStorage. The QR
    // generated by the server embeds the URL with the pair code as a query
    // param, so a fresh scan in any browser auto-populates this.
    function loadPair() {
        var u = new URL(location.href);
        var fromUrl = (u.searchParams.get('pair') || '').toUpperCase().trim();
        if (fromUrl) {
            try { localStorage.setItem('vior_pair', fromUrl); } catch (_) {}
            return fromUrl;
        }
        try { return (localStorage.getItem('vior_pair') || '').toUpperCase().trim(); } catch (_) { return ''; }
    }
    ...
    function connect() {
        ...
        ws.onmessage = function (evt) {
            ...
            if (msg.type === 'ready') {
                ...
                } else if (msg.data.code === 'pair_mismatch') {
    ...
    function showError(msg, needsPair) {
        errorMsg.textContent = msg;
        if (pairRow) {
            if (needsPair) pairRow.classList.remove('hidden');
            else pairRow.classList.add('hidden');
        }
        if (needsPair && pairInput) setTimeout(function () { try { pairInput.focus(); } catch (_) {} }, 50);
    ...
    retryBtn.addEventListener('click', function () {
        if (pairInput && pairInput.value) {
            pairCode = pairInput.value.toUpperCase().trim();
            try { localStorage.setItem('vior_pair', pairCode); } catch (_) {}
        }
        reconnectDelay = 1000;
        connect();
    });
    if (pairInput) {
        pairInput.addEventListener('keydown', function (e) {
            if (e.key === 'Enter') { e.preventDefault(); retryBtn.click(); }
        });
    }
